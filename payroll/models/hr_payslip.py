...
#     _inherit = 'hr.payslip'
    ...
    #     worked_days = list(res) if res else []

    #     for contract in contract_ids:
    #         employee = contract.employee_id
    #         # Search approved leaves within the payslip date range
    ...
    #                 'sequence': 100,
    #                 'code': leave_type.code or leave_type.name[:5].upper(),
    #                 'number_of_days': number_of_days,
    #                 'number_of_hours': number_of_days * 8,  # assuming 8h per day
...
class HrPayslip(models.Model):
    ...
    line_ids = fields.One2many(
        ...
        string='Salary Computation', readonly=True
    )
    ...
    def _get_worked_day_lines(self):
        ...
        for entry in work_entries:
            worked_days_lines.append({
                'name': entry.name,
                'sequence': entry.work_entry_type_id.sequence,
                'code': entry.work_entry_type_id.code,
                'number_of_days': (entry.date_stop - entry.date_start).days + 1,
                'number_of_hours': entry.duration,
                'contract_id': self.contract_id.id,
                'work_entry_type_id': entry.work_entry_type_id.id,
            })

        return {
            'worked_days_line_ids': [(0, 0, line) for line in worked_days_lines]
        }
    def compute_sheet(self):
        res = super().compute_sheet()
        for payslip in self:
            payslip.worked_days_line_ids = [(5, 0, 0)]  # Clear existing
            worked_days_result = payslip._get_worked_day_lines()
            payslip.worked_days_line_ids = worked_days_result.get('worked_days_line_ids', [])
        return res
    ...
    # @api.model
    # def _get_worked_day_lines(self, contracts, date_from, date_to):
    #     result = []

    #     for contract in contracts:
    #         employee = contract.employee_id

    #         # Get validated time off for this employee during the payslip period
    ...
    #             leave_type = leave.holiday_status_id
    #         for leave in leaves:
    #             _logger.warning("Leave: ID=%s, Type=%s, From=%s, To=%s", leave.id, leave.holiday_status_id.name, leave.request_date_from, leave.request_date_to)


    #             # Calculate actual overlapping days between leave and payslip period
    ...
    #         # ✅ Add normal working days
    #         work_data = employee._get_work_days_data_batch(
    #             date_from, date_to, calendar=contract.resource_calendar_id
    #         )[contract.id]
    ...
    #         # ✅ Group leave totals by leave type
    #         leave_summary = {}
    #         for leave in leaves:
    #             leave_type = leave.holiday_status_id
    #             leave_start = max(leave.request_date_from, date_from)
    ...
    #             leave_summary[key]['number_of_days'] += leave_days
    #             leave_summary[key]['number_of_hours'] += leave_days * 8

    #             # Optional debug log
    #             _logger.warning(
    #                 "Leave: %s - %s to %s (%s days)",
    #                 leave_type.name, leave_start, leave_end, leave_days
    #             )

    #         # ✅ Add leave lines
    #         result.extend(leave_summary.values())
    ...
    def _get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        #         _logger.warning("Leave: %s - %s to %s (%s days)", leave_type.name, leave_start, leave_end, leave_days)
        #         _logger.warning("Leaves found for %s: %s", employee.name, [l.name for l in leaves])

        #     result.extend(leave_summary.values())

        # return result
 
    @api.onchange('struct_id', 'employee_id', 'date_from', 'date_to', 'contract_id')
    def _onchange_structure_compute_sheet(self):
        for slip in self:
            # Only compute if the record is already saved
            if slip.id and slip.struct_id and slip.employee_id:
                slip.with_context(skip_onchange=True).compute_sheet()
 
    @api.model
    def create(self, vals):
        slips = super().create(vals)
        slips.compute_sheet()
        return slips
 
    def write(self, vals):
        res = super().write(vals)
        if any(field in vals for field in ['struct_id', 'employee_id', 'date_from', 'date_to', 'contract_id']):
            self.compute_sheet()
        return res
